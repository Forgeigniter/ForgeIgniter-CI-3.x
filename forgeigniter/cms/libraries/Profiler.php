...
class CI_Profiler extends CI_Loader {
	...
	protected $_compile_files;
	protected $_compile_console;
	protected $_compile_userdata;
	...
	protected $_compile_eloquent;
	protected $_compile_uri_string;
	protected $_compile_view_data;
	...
	public function __construct($config = array())
	{
		...
		foreach ($this->_available_sections as $section)
		{
			if ( ! isset($config[$section]))
			{
				...
				$compile_section = "_compile_{$section}";
				$this->$compile_section = TRUE;
			}
		}
	...
	public function set_sections($config)
	{
		foreach ($config as $method => $enable)
		{
			...
			{
				$compile_method = "_compile_{$method}";
				$this->$compile_method = ($enable !== FALSE) ? TRUE : FALSE;
			}
		}
	}
	...
	protected function _compile_config()
	{
		...
		foreach ($this->CI->config->config as $config=>$val)
		{
			...
			$output[$config] = htmlspecialchars($val ?? '', ENT_QUOTES, ini_get('default_charset'));
		}
