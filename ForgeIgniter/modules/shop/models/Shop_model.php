...
class Shop_model extends CI_Model
{
    ...
    public $last_error = '';
    public $response = '';
    public $response_log_file = '';
    public $response_log = '';
    public $response_data = array();	// array contains the POST values for IPN
    public $fields = array();		// array holds the fields to submit to paypal
    public $shopShippingTable = array();
    public $siteVars = array();
    ...
    public function __construct()
    {
        ...
        if (0) {
            ...
            if ($this->site->config['shopGateway'] == 'authorize') {
                $this->gateway_url = 'https://test.authorize.net/gateway/transact.dll';
            }
            if ($this->site->config['shopGateway'] == 'rbsworldpay') {
                $this->gateway_url = 'https://secure-test.wp3.rbsworldpay.com/wcc/purchase';
            }
            if ($this->site->config['shopGateway'] == 'sagepay') {
        ...
        // set gateway url
            ...
            if ($this->site->config['shopGateway'] == 'rbsworldpay') {
                $this->gateway_url = 'https://secure.wp3.rbsworldpay.com/wcc/purchase';
            }
            if ($this->site->config['shopGateway'] == 'sagepay') {
    ...
    public function get_category_by_reference($catRef = '', $parentRef = '')
    {
        // get parent
    ...
    public function get_cats_for_product($productID)
    {
    ...
    public function get_cat_ids_for_product($productID)
    {
        // get cats for this product
        $this->db->join('shop_cats', 'shop_catmap.catID = shop_cats.catID', 'left');
        $this->db->order_by('catOrder');
    ...
    public function update_cats($productID, $catsArray = '')
    {
        // delete cats
    ...
    public function get_band($bandID)
    {
        // default where
        $this->db->where(array('siteID' => $this->siteID));
    ...
    public function get_band_by_multiplier($multiplier = '')
    {
        // default where
    ...
    public function get_modifiers($multiplier = '')
    {
        // default where
    ...
    public function get_modifier($modifierID)
    {
        // default where
        $this->db->where('shop_modifiers.siteID', $this->siteID);
    ...
    public function get_modifier_by_multiplier($multiplier = '')
    {
        // default where
        $this->db->where('shop_modifiers.siteID', $this->siteID);
    ...
    public function get_postage($postageID)
    {
        // default where
        $this->db->where(array('siteID' => $this->siteID));
    ...
    public function get_products($catID = '', $search = '', $featured = false, $limit = '')
    {
    ...
    public function get_products_by_tag($tag = '', $limit = '')
    {
        // set limit from uri if set
        $limit = (!$limit && $limit != 'all') ? $this->siteVars['shopItemsPerPage'] : $limit;
    ...
    public function get_catmap_product_ids($catID)
    {
    ...
    public function get_latest_products($catSafe = '', $limit = 3)
    {
    ...
    public function get_latest_featured_products($limit = 3)
    {
        $this->db->where('siteID', $this->siteID);
        $this->db->where('deleted', 0);
        $this->db->where('featured', 'Y');
    ...
    public function get_popular_products($limit = 3)
    {
        // where
    ...
    public function get_most_viewed_products($limit = 3)
    {
        $this->db->where(array('siteID' => $this->siteID, 'deleted' => 0, 'published' => 1));
    ...
    public function get_similar_products($productID = '', $catID = '', $limit)
    {
        if (!$catID || !$productID) {
            return false;
        }
    ...
    public function get_discounts($code)
    {
        // default where
    ...
    public function get_discount($discountID)
    {
        // default where
        $this->db->where(array('siteID' => $this->siteID));
    ...
    public function get_upsell($upsellID)
    {
        // default where
        $this->db->where(array('siteID' => $this->siteID, 'deleted' => 0));
    ...
    public function get_variations($productID, $type = '')
    {
    ...
    public function get_variation($variationID)
    {
    ...
    public function unpack_item($key, $quantity)
    {
    ...
    public function get_product_ids_in_cart()
    {
        // get session data for cart
        $cartSession = $this->session->userdata('cart');
    ...
    public function add_to_cart($productID, $quantity = 1, $variation1 = '', $variation2 = '', $variation3 = '')
    {
        if ($quantity < 1) {
            $quantity = 1;
        }
    ...
    public function update_cart($key, $quantity)
    {
        if ($quantity) {
            $cart = $this->session->userdata('cart');
            $key = $this->core->decode($key);
    ...
    public function add_variation($productID, $type, $variation = '', $varPrice = 0)
    {
    ...
    public function clear_variations($productID = '')
    {
        if ($productID) {
            $this->db->where('productID', $productID);
    ...
    public function get_orders($status = '', $userID = '', $search = '')
    {
    ...
    public function get_order($transactionID, $userID = '')
    {
    ...
    public function get_item_orders($transactionID)
    {
    ...
    public function get_subscription_by_ref($subscriptionRef)
    {
        $this->db->where('siteID', $this->siteID);
    ...
    public function get_subscribers($q = '')
    {
        $this->db->where('siteID', $this->siteID);
    ...
    public function get_num_subscribers($subscriptionID)
    {
        $this->db->where('siteID', $this->siteID);
        $this->db->where('subscriptionID', $subscriptionID);
    ...
    public function get_subscriber($subscriberID)
    {
        $this->db->where('siteID', $this->siteID);
    ...
    public function get_sub_payments($userID)
    {
        $this->db->where('t1.siteID', $this->siteID, false);
    ...
    public function get_sub_payment($paymentID)
    {
        $this->db->where('t1.siteID', $this->siteID, false);
    ...
    public function minus_stock($productID, $quantity)
    {
    ...
    public function update_order($transactionID)
    {
        $this->db->set('paid', 1);
    ...
    public function validate_paypalpro()
    {
        // parse the paypal URL
        $url_parsed = parse_url($this->gateway_url);
    ...
    public function validate_authorize()
    {
        // parse the paypal URL
        $url_parsed = parse_url($this->gateway_url);
    ...
    public function validate_rbsworldpay()
    {
    ...
    public function validate_sagepay()
    {
        // get post	from worldpay
        if ($_POST) {
            $output = '';
    ...
    public function validate_sub_payment()
    {
    ...
    public function update_subscriber($subscriberID = '')
    {
        if ($this->site->config['shopGateway'] == 'paypal') {
    ...
    public function cancel_subscriber()
    {
        if ($this->site->config['shopGateway'] == 'paypal') {
            $referenceID = $this->response_data['subscr_id'];
        } elseif ($this->site->config['shopGateway'] == 'rbsworldpay') {
    ...
    public function log_response_results($success)
    {
    ...
    public function upgrade_user($userID, $plan = '')
    {
    ...
    public function downgrade_user($userID)
    {
        // update users based on subscriber action
        if ($this->site->config['subscriberAction'] > 0) {
    ...
    public function lookup_user_by_email($email)
    {
        // default where
        $this->db->where('siteID', $this->siteID);
        $this->db->where('email', $email);
